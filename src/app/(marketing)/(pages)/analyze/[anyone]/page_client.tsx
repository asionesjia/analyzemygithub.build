      ...
      <div className="space-y-4 px-4 pt-4 md:space-y-8 md:px-10 md:pt-10">
        <BlurInEffect index={0}>
          <div className="pb-4 text-3xl font-semibold sm:text-5xl md:pb-8">
            {isAnalyzed ? 'Your Analysis Completed ✨' : 'Prepare for analysing GitHub'}
          </div>
          <LineSeparator />
        </BlurInEffect>
...
const Page_client = ({ username }: { username: string }) => {
  ...
  const handleAnalyze = useCallback(async () => {
    try {
      ...
      setLoading(true)
      for await (const value of iterateStreamResponse(publicAnalyzeAction(username))) {
        console.log(value)
        // @ts-ignore
